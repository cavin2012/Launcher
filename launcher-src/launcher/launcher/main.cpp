...
static void did_net_state_changed(const char* iface, bool connected, uint32_t ipv4, void* user)
{
	game_instance* instance = reinterpret_cast<game_instance*>(user);
	instance->did_net_state_changed(iface, connected, ipv4);
}
...
game_instance::game_instance(rtc::PhysicalSocketServer& ss, int argc, char** argv)
	...
	, pble_(this)
{
	kosSetNetStateChanged(::did_net_state_changed, this);
}
...
void game_instance::app_load_settings_config(const config& cfg)
{
	...
	const version_info min_libkosapi_ver("1.0.1-20201011");
