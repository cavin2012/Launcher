...
void texplorer_slot::rexplorer_click_edit(tbutton& widget)
{
	...
	if (selected == copy) {
		...
		gui2::run_with_progress_dlg(title, cliprdr_msgid_2_str(cliprdr_msgid_paste, null_str), boost::bind(&net::trdpd_manager::hdrop_paste, &rdpd_mgr_, _1, rexplorer->current_dir(), &err_code, err_msg, sizeof(err_msg)), 0, "misc/remove.png");
