...
public:
	...
	void SetUp(uint32_t ipaddr, int port, void* decoder);
...
private:
	...
	std::string check_launcher_version(const char* hostname);

protected:
	base::Thread& thread_;
	rtc::MessageHandler& desktop_;
...
public:
	...
	void start(uint32_t ipaddr, int port, void* decoder, rtc::MessageHandler& desktop, twebrtc_send_helper& send_helper);
	void stop();

private:
	void did_set_event();
	void start_internal(uint32_t ipaddr, int port, void* decoder, rtc::MessageHandler* desktop_ptr, twebrtc_send_helper* send_helper_ptr);
	void stop_internal();
