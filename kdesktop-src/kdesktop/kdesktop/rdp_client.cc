...
RdpClientRose::RdpClientRose(base::Thread& thread, rtc::MessageHandler& desktop, twebrtc_send_helper& send_helper)
	: thread_(thread)
	, desktop_(desktop)
...
{
	DCHECK_CALLED_ON_VALID_THREAD(client_->thread_checker_);
...
void RdpClientRose::SetUp(uint32_t ipv4, int port, void* decoder)
{
...
void RdpClientRose::send_startup_msg(uint32_t ticks, const std::string& msg, bool fail, int rdpstatus)
{
	...
	if (lock.can_send()) {
		...
		instance->sdl_thread().Send(RTC_FROM_HERE, &desktop_, gui2::tdesktop::MSG_STARTUP_MSG, pdata);
	}
}

std::string RdpClientRose::check_launcher_version(const char* hostname)
{
	version_info launcher_ver;
	if (hostname != nullptr) {
		int len = 0;
		const char* ptr = app_version_from_hostname(hostname, &len);
		if (len > 0) {
			VALIDATE(ptr != nullptr, null_str);
			launcher_ver = version_info(std::string(ptr, len));
		}
	}

	std::string err;
	if (!launcher_ver.is_rose_recommended() || launcher_ver < game_config::min_launcher_ver) {
		utils::string_map symbols;

		symbols["cur"] = launcher_ver.str(true);
		symbols["min"] = game_config::min_launcher_ver.str(true);
		err = vgettext2("Launcher's version($cur) must >= $min", symbols);
	}
	return err;
}
...
bool RdpClientRose::OnConnectionComplete()
{
	freerdp* instance = freerdp_;
/*
	// instance->context->settings->ClientHostname is kdesktop's hostname, it requrie launcher's hostname.
	// now I hasn't method to get launcher's hostname, implete to check min_launcher_version in next version.
	const std::string ver_err = check_launcher_version(instance->context->settings->ClientHostname);
	if (!ver_err.empty()) {
		send_startup_msg(SDL_GetTicks(), ver_err, true, rdpstatus_connectionclosed);
		client_->Close(net::OK);
		return false;
	}
*/
...
void trdp_manager::start_internal(uint32_t ipaddr, int port, void* decoder, rtc::MessageHandler* desktop_ptr, twebrtc_send_helper* send_helper_ptr)
{
	{
		tauto_destruct_executor destruct_executor(boost::bind(&trdp_manager::did_set_event, this));
		delegate_.reset(new RdpClientRose(*thread_.get(), *desktop_ptr, *send_helper_ptr));
	}
	// Setup will send_startup_msg to main-thread, avoid deallock, main-thread must not wait Setup completed.
	delegate_->SetUp(ipaddr, port, decoder);
}
...
void trdp_manager::start(uint32_t ipaddr, int port, void* decoder, rtc::MessageHandler& desktop, twebrtc_send_helper& send_helper)
{
	...
	thread_->task_runner()->PostTask(FROM_HERE, base::BindOnce(&trdp_manager::start_internal, base::Unretained(this), ipaddr, port, decoder, &desktop, &send_helper));
