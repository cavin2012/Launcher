...
NetdListener::NetdListener(int sock) :
                 ...
                 , mWithSeq(false)
                 , mNetStateChanged(nullptr)
                 , mNetStateChangedUser(nullptr)
...
void NetdListener::runCommand(int argc, char ** argv)
{
    ...
    ALOGD("runCommand, code: %i, cmd/cmdNum: %s", code, cmd.c_str());

    if (code == NetdResponseCode::InterfaceAddressChange) {
        ...
        int prefixlen = 0;
        const uint32_t ipv4 = splitIpPrefixlen(argv[3], &prefixlen);
        ALOGD("runCommand, ipv4: 0x%08x, prefixlen: %i, %s, mNetStateChanged: 0x%p", ipv4, prefixlen, updated? "connected": "disconnected", mNetStateChanged);
        if (mNetStateChanged != nullptr) {
            mNetStateChanged(argv[4], updated, ipv4, mNetStateChangedUser);
        }
        return;
    ...
    ALOGD("runCommand, 2, cmd/cmdNum: %i", cmdNum);
    if (cmdNum < 0) {
        return;
    }
...
NDK_EXPORT void kosSetNetStateChanged(fkosNetStateChanged did, void* user)
{
    if (makeSureNetdListener() == nullptr) {
        ALOGE("kosSetNetStateChanged, netdl is nullptr");
        return;
    }
    netdl->setNetStateChanged(did, user);
}
